"""
&ILOR COINS — Discord Economy Bot
Required env vars:
...
async def daily(interaction: discord.Interaction):
    ...
    if on_cd:
        return await interaction.response.send_message(
            embed=discord.Embed(title="# claimed!", description=f"Come back in {fmt_time(rem)}", color=RED)
        )
    data = get_user(uid)
    streak = data[7] + 1
...
async def mine(interaction: discord.Interaction):
    ...
    if on_cd:
        return await interaction.response.send_message(
            embed=discord.Embed(title="& Still Mining", description=f"Wait {fmt_time(rem)}", color=RED)
        )
    luck = get_mult(uid, "luck")
    coins = get_mult(uid, "coins")
    inv = get_inv(uid)
    ...
    add_wallet(uid, amount)
    set_cd(uid, "mine")
    embed = discord.Embed(title="& Mining Results!", color=0x8B4513)
...
async def crime(interaction: discord.Interaction):
    ...
    if random.random() < fail_chance:
        ...
        embed.add_field(name="Result", value=f"{act[1]}", inline=False)
...
async def rob(interaction: discord.Interaction, user: discord.Member):
    ...
    if on_cd:
        return await interaction.response.send_message(
            embed=discord.Embed(title="#iding", description=f"Wait {fmt_time(rem)}", color=RED)
        )
    robber = get_user(rid)
    victim = get_user(vid)
    if robber[1] < 100:
...
async def gamble(interaction: discord.Interaction, amount: str):
    ...
    if on_cd:
        return await interaction.response.send_message(f"# {fmt_time(rem)}", ephemeral=True)
    set_cd(uid, "gamble")
...
async def shop(interaction: discord.Interaction):
    ...
    conn.close()
    embed = discord.Embed(title="&or Shop", description="Use /buy <item> to purchase!", color=GOLD)
    for item in items:
...
async def leaderboard(interaction: discord.Interaction):
    ...
    conn.close()
    embed = discord.Embed(title="&or Coins Leaderboard", color=GOLD)
    medals = ["🥇","🥈","🥉"]
